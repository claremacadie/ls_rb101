...
# my solution for bonus feature 3 - AI defense move
...
def consecutive_lines(brd)
  consecutive_lines = WINNING_LINES.each_with_object([]) do |line, arr|
    arr << line if brd.values_at(*line).count(PLAYER_MARKER) == 2
  end
  consecutive_lines
end

def player_has_consecutive_squares?(brd)
  consecutive_lines(brd).empty? ? false : true
end

def computer_defense_move(brd)
  consecutive_lines(brd).each do |arr|
    arr.each do |square|
      return square if brd[square] == ' '
    end
  end
  nil
end
...
def computer_places_piece(brd)
  square = if player_has_consecutive_squares?(brd) && computer_defense_move(brd)
