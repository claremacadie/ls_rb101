...
# solution for user to choose to go first, computer to go first or random.
...
def decide_who_goes_first
  ...
  when 'p' then 'n'
  when 'c' then 'y'
  when 'r' then ['y', 'n'].sample
  end
end

def declare_who_goes_first(skip)
  if skip == 'y'
...
loop do
  ...
  skip = decide_who_goes_first
  declare_who_goes_first(skip)
